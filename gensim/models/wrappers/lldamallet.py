...
r"""Python wrapper for `Labeled Latent Dirichlet Allocation (L-LDA) <https://www-nlp.stanford.edu/cmanning/papers/llda-emnlp09.pdf>`_
from `MALLET, the Java topic modelling toolkit <http://mallet.cs.umass.edu/>`_

This module allows both L-LDA model estimation from a training corpus and inference of topic distribution on new,
unseen documents, using an (optimized version of) collapsed gibbs sampling from MALLET.

Notes
-----
MALLET's Labeled LDA training requires :math:`O(corpus\_words)` of memory, keeping the entire corpus in RAM.
If you find yourself running out of memory, decrease the `workers` constructor parameter.
...
.. sourcecode:: pycon
    ...
    >>> from gensim.models.wrappers import LLLdaMallet
    >>>
    >>> path_to_mallet_binary = "/path/to/mallet/binary"
    >>> model = LLLdaMallet(path_to_mallet_binary, corpus=common_corpus, num_topics=20, id2word=common_dictionary)
    >>> vector = model[common_corpus[0]]  # L-LDA topics of a documents
...
class LLdaMallet(utils.SaveLoad, basemodel.BaseTopicModel):
    """Python wrapper for Labeled LDA using `MALLET <http://mallet.cs.umass.edu/>`_.
    ...
    Warnings
    --------
    This is **only** python wrapper for `MALLET Labeled LDA <http://mallet.cs.umass.edu/>`_,
    you need to install original implementation first and pass the path to binary to ``mallet_path``.

    """
    def __init__(self, mallet_path, corpus=None, labels=None, id2word=None, workers=4, iterations=1000,
        ...
        labels : iterable of iterable of str
        ...
        if self.num_terms == 0:
            raise ValueError("cannot compute L-LDA over an empty collection (no terms)")
        
        self.labels = labels
        # num_topics gets calculated by number of unique labels
        self.num_topics = len(set([label for doc in labels for label in doc]))
    ...
    def corpus2lldamallet(self, corpus, labels, file_like):
        """Convert `corpus` and `labels` to Mallet format and write it to `file_like` descriptor.

        Format ::
            LDA:
            document id[SPACE]label (not used)[SPACE]whitespace delimited utf8-encoded tokens[NEWLINE]
            L-LDA:
            document id[TAB]whitespace delimited utf8-encoded labels[TAB]whitespace delimited utf8-encoded tokens[NEWLINE]
        ...
        """
        for docno, (doc, label_list) in enumerate(zip(corpus, labels)):
            if self.id2word:
            ...
            file_like.write(utils.to_utf8("%s\t%s\t%s\n" % (docno, ' '.join(label_list),' '.join(tokens))))
    ...
    def convert_input(self, corpus, infer=False, serialize_corpus=True):
        ...
        if serialize_corpus:
            ...
            with utils.open(self.fcorpustxt(), 'wb') as fout:
                self.corpus2lldamallet(corpus, self.labels, fout)
        ...
        # See http://www.mimno.org/articles/labelsandpatterns for usage
        cmd = \
            self.mallet_path + \
            " import-file --preserve-case --keep-sequence " \
            "--label-as-features --line-regex \"([^\\t]+)\\t([^\\t]+)\\t(.*)\" " \
            "--remove-stopwords --token-regex \"\\S+\" --input %s --output %s"
    ...
    def train(self, corpus, labels):
        ...
        # --num-topics is not supported because there is exactly one topic per unique label
        # '--use-symmetric-alpha %s --optimize-interval %s --optimize-burn-in %s '\ # TODO: Add support
        # --num-threads %s  # TODO: Add support
        cmd = self.mallet_path + ' run cc.mallet.topics.LabeledLDA --input %s  --alpha %s  --beta %s '\
            '--output-state %s --output-doc-topics %s --output-topic-keys %s '\
            '--num-iterations %s --inferencer-filename %s --doc-topics-threshold %s  --random-seed %s'

        # Mallet's LDA alpha parameter is the alphaSum parameter
        # However, Labeled LDA (L-LDA) uses gensim's alpha parameter style
        mallet_alpha = sum(self.alpha) / self.num_topics
        # Should be the same (only symmetrical alphas are allowed to pass as a parameter)
        assert mallet_alpha == self.alpha[0]
        ...
        # self.use_symmetric_alpha, self.optimize_interval, self.optimize_burn_in # TODO: Add support
        # --num-threads %s  # TODO: Add support
        cmd = cmd % (
            self.fcorpusmallet(), mallet_alpha, mallet_beta, self.fstate(), self.fdoctopics(),
            self.ftopickeys(), self.iterations,
            self.finferencer(), self.topic_threshold, str(self.random_seed)
        )
        # NOTE "--keep-sequence-bigrams" / "--use-ngrams true" poorer results + runs out of memory
        logger.info("Training MALLET L-LDA with %s", cmd)
        check_output(args=cmd, shell=True)
    ...
    def __getitem__(self, bow, iterations=100):
        ...
        -------
        list of (int, float)
            L-LDA vector for document as sequence of (topic_id, topic_probability) **OR**
        list of list of (int, float)
            L-LDA vectors for corpus in same format.
        ...
        logger.info("Inferring topics with MALLET L-LDA '%s'", cmd)
        check_output(args=cmd, shell=True)
        result = list(self.read_doctopics(self.fdoctopics() + '.infer'))
        return result if is_corpus else result[0]

    def load_word_topics(self):
        """Load words X topics matrix from :meth:`gensim.models.wrappers.LLdaMallet.LLdaMallet.fstate` file.
    ...
    def load_document_topics(self):
        """Load document topics from :meth:`gensim.models.wrappers.LLdaMallet.LLdaMallet.fdoctopics` file.
        Shortcut for :meth:`gensim.models.wrappers.LLdaMallet.LLdaMallet.read_doctopics`.

        Returns
        -------
        iterator of list of (int, float)
            Sequence of L-LDA vectors for documents.
    ...
    def load(cls, *args, **kwargs):
        """Load a previously saved LLdaMallet class.
        """
        model = super(LLdaMallet, cls).load(*args, **kwargs)
    ...
    def __str__(self):
        ...
        """
        return "LLdaMallet(num_terms=%s, num_topics=%s, num_iterations=%s, random_state=%s)" % (
...
def malletmodel2ldamodel(mallet_model, gamma_threshold=0.001, iterations=50):
    """Convert :class:`~gensim.models.wrappers.LLdaMallet.LLdaMallet` to :class:`~gensim.models.ldamodel.LdaModel`.

    This works by copying the training model weights (alpha, beta...) from a trained mallet model into the gensim model.

    Parameters
    ----------
    mallet_model : :class:`~gensim.models.wrappers.LLdaMallet.LLdaMallet`
